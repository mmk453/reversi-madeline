...
*/
    ...
    socket.on('join_room', (payload) => {
        serverLog('Server recevied a command', '\'join_room\'',JSON.stringify(payload));
        ...
        if ((typeof username == 'undefined') || (username === null)){
            response = {};
            response.result = 'fail';
            response.message = 'client did not send a username to join the chat room';
            socket.emit('join_room_response',response);
            serverLog('join_room command failed', JSON.stringify(response));
            return;
        }
        ...
        io.in(room).fetchSockets().then((sockets)=>{
            ...
            /* Socket did join room */
               ...
               for (const member of sockets){
                   ...
                   if (room !== "Lobby") {
                       send_game_update(socket, room, 'initiate update');
                   }
                }   
            }
        });
    ...
    socket.on('invite', (payload) => {
        serverLog('Server recevied a command', '\'invite\'',JSON.stringify(payload));
        /* Check that the data coming from the client is good */
        if ((typeof payload == 'undefined') || (payload === null)){
            response = {};
            response.result = 'fail';
        ...
        io.in(room).allSockets().then((sockets)=>{
            ...
            /* Invitee is in the room */
                ...
                serverLog('invite comman succeeded', JSON.stringify(response));
            }
        });
    });
    ...
    socket.on('uninvite', (payload) => {
        serverLog('Server recevied a command', '\'uninvite\'',JSON.stringify(payload));
        /* Check that the data coming from the client is good */
        if ((typeof payload == 'undefined') || (payload === null)){
            response = {};
            response.result = 'fail';
    ...
    socket.on('game_start', (payload) => {
        serverLog('Server recevied a command', '\'game_start\'',JSON.stringify(payload));
        /* Check that the data coming from the client is good */
        if ((typeof payload == 'undefined') || (payload === null)){
            response = {};
            response.result = 'fail';
...
socket.on('send_chat_message', (payload) => {
    ...
    socket.on('play_token', (payload) => {
        ...
        if(
            (( game.whose_turn === 'ue') && (game.player_ue.socket != socket.id)) ||
        ...
        /* Execute the move */
        if(color === 'ue') {
            game.board[row][column] = 'u';
            flip_tokens('u',row,column,game.board);
...
function create_new_game() {
    let new_game = {};
    new_game.player_ue = {};
    new_game.player_ue.socket = "";
    new_game.player_ue.username = "";
...
function send_game_update(socket, game_id, message) {
    ...
    io.of('/').to(game_id).allSockets().then((sockets) => {
        ...
        if(sockets.size >=1) {
            ...
            if ((games[game_id].player_ue.socket != first) &&
                (games[game_id].player_black.socket != first)) {
                 /* Player does not have a color yet */
                 if (games[game_id].player_ue.socket === ""){
                     /* This player should be blue */
                     console.log("Blue is assigned to: "+first);
                     games[game_id].player_ue.socket = first;
                    games[game_id].player_ue.username = players[first].username;
                }
                    ...
                    /* This player shouuld be kicked out */
                     console.log("Kicking "+ first + " out of gmae: " + game_id);
        ...
        if(sockets.size >=2) {
            ...
            if ((games[game_id].player_ue.socket != second) &&
                (games[game_id].player_black.socket != second)) {
                 /* Player does not have a color yet */
                 if (games[game_id].player_ue.socket === ""){
                     /* This player should be blue */
                     console.log("Blue is assigned to: "+second);
                     games[game_id].player_ue.socket = second;
                    games[game_id].player_ue.username = players[second].username;
                }
                 else if (games[game_id].player_black.socket === "") {
                    /* This player should be black */
                    ...
                    /* This player shouuld be kicked out */
                     console.log("Kicking "+ second + " out of gmae: " + game_id);
    ...
    for (let row = 0; row < 8; row++){
        for (let column = 0; column < 8; column++){
            ...
            if (games[game_id].board[row][column] === 'u') {
                uesum++;
            }
